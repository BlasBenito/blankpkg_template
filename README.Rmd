...
- `setup_rcpp_infrastructure.R` - Configure Rcpp infrastructure for C++ code
- `install_dev_dependencies.R` - Install all development packages
- `install_linter_jarl.R` - Display jarl linter installation instructions
- `install_formatter_air.R` - Install air formatter (wraps usethis::use_air())

**Daily Development (3 functions)**
- `check.R` and `check_full.R` - Quick vs comprehensive checks (replaced `daily_document_and_check.R`)
- `daily_test.R` - Run quick test suite
- `daily_load_all.R` - Load package for interactive development

**Testing Suite (3 functions)**
- `test_run_all.R` - Run complete test suite with detailed output
- `test_with_coverage.R` - Run tests with HTML coverage report
- `test_spelling.R` - Check spelling in all documentation

**Checking Suite (5 functions)**
- `check.R` and `check_full.R` - Local R CMD check (quick vs comprehensive)
- `check_good_practice.R` - Analyze package for R best practices
- `check_win_devel.R` - Submit to Windows R-devel builder
- `check_mac_release.R` - Submit to macOS R-release builder
- `check_rhub_multi_platform.R` - Multi-platform checks via R-Hub (20+ platforms)
...
- `pkgdown_build_site.R` - Build complete package website
- `pkgdown_customize_site.R` - Interactive guide for customizing _pkgdown.yml
...
- `analyze_code_coverage.R` - Calculate and display test coverage
- `analyze_code_quality.R` - Static code analysis with codetools
- `analyze_dependencies.R` - Create interactive dependency network graph
- `analyze_package_structure.R` - Analyze package organization and metrics
- `analyze_performance.R` - Benchmarking and profiling template (TEMPLATE)

**Development Helpers (2 functions)**
- `create_example_data.R` - Template for creating package datasets (TEMPLATE)
- `create_example_function.R` - Template for creating package functions (TEMPLATE)
